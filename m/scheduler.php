...
?>
<!doctype html>
<?php   include('mysql_settings3.php')?>
<html>
<head>
		<meta charset="UTF-8">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
        	<div class="container">
          	...
          	<select style="width:50%;float:left;" name="time_start_h">
          		<?php
          		for($i=0;$i<24;$i++) {
          			?>
          	...
          	<select style="width:50%;float:left;" name="time_start_m">
          		<?php
          		for($i=0;$i<60;$i+=5) {
          			?>
...
<div id="sliderWrap2" style = "margin-top: 10px;">
			...
			<li>
				<div>
					...
					<table class="table table-bordered table-hover table-condesned pull-right" style="width:100%;">
						
						<tr style="height:40px;">
						
						<td style="width:30%;height:40px;">시간</td>
							
						<td style="width:70%;height:40px;">스케줄</td>
						</tr>
						<?php
						...
						<tr style="height:40px;">
							...
							<?php if($t % 30 == 0){ 
								$sda = $row[0]; // time_start
								$eda = $row[1]; //time_end
								?>
								<td style="width:30%;" rowspan="6">
								<?php 
								echo ((int)($t/60))."시".($t-((int)($t/60))*60)."분 ~ ".((int)(($t+30)/60))."시".(($t+30)-((int)(($t+30)/60))*60)."분 ";	
								...
								<?php
									$s = (int)$row[0]; //time_start
									
									$e = (int)$row[1]; //time_end
									if($t==$e) {
										$row = mysqli_fetch_array($result,MYSQLI_BOTH);
										$s = (int)$row[0];// time_start
									...
									if($s==$t) {
										...
										if($cc >= 2) echo "rowspan='$cc'>";
										else echo ">";
											echo $row[3]; //schedule_name
											if($e-$s > 5) echo "<br />";
											else echo "&nbsp;/&nbsp;";
											echo $row[4];//schedule_des
